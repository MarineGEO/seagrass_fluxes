...
In lieu of this procedure, which requires specialized equipment, seagrass blades could be scraped, dried, and weighed to obtain an estimate of total fouling biomass. This procedure would include organisms other than epiphytes, like sessile animals and detritus, but arguably the organisms are feeding on these resources as well.
...
The final diet matrix consists of an _s_ -by- _s_ matrix of _s_ species, with a 1 if the species in column _j_ consumes the organisms in row _i_, and a 0 if they do not interact.
...
This equation could further incorporate the effects of temperature on metabolic rates (eqn. 4, Brown et al. (2004)). In fact, comparing estimated fluxes with and without a scaling parameter for temperature would be very enlightening, allowing us to disentangle the direct (with) and indirect (without) effects of temperature on metabolic losses. For purposes of simplicity, we will proceed with the estimates derived from body size alone.
...
We see that fluxes are quite low in the colder winter months (March, April, November) but increase rapidly in the spring though mid-summer, when production is highest (July). There is a dip in August, when eelgrass naturally senesces in this part of the world. There is a slight bump in the early fall due to the slight resurgence in seagrass during this period, but by then much of the faunal community has emigrated from the Bay. Notably and expectedly, fluxes are lower to predators than to herbivores due to decreasing efficiency with trophic level.

It is easy to see how this plot would function for the proposed work, with a space-for-time substitution. The x-axis could be latitude or any other spatially-structured variable.

We can further relate these values to the biotic and abiotic properties of the ecosystem. For example, the original derivation of this framework focused on how community diversity influenced fluxes. Theory suggests that more diverse communities are more efficient at using available resources (Cardinale et al. (2012)), translating to higher energy flux.
...
We can use linear models to test the independent effects of each of these properties--and others, like salinity--while controlling for the contributions of the others and accounting for temporal autocorrelation (months within a year, among years). This analysis will tell us whether biodiversity is still important given the influence of temperature.
...
kable(rsquared(herbivory.mod), caption = "Model R^2^ values for fluxes to herbivores.")
```
...
kable(rsquared(predation.mod), caption = "Model R^2^ values for fluxes to carnivores.")
```
